...
int main() {
    ...
    Prism::Camera cam(lookfrom, lookat, vup, 2.0, 2.0, 2.0 * aspect_ratio, image_height,
                      image_width);
    ...
    auto material_esfera_1 =
        std::make_shared<Prism::Material>(Prism::Color(1.0, 0.3, 0.3)); // Vermelho/Rosa
    auto material_esfera_2 = std::make_shared<Prism::Material>(Prism::Color(0.3, 0.3, 1.0)); // Azul

    // Adição dos Objetos à Cena

    scene.addObject(std::make_unique<Prism::Plane>(Prism::Point3(0, -0.5, 0),
                                                   Prism::Vector3(0, 1, 0), material_chao));

    scene.addObject(
        std::make_unique<Prism::Sphere>(Prism::Point3(0, 0, -1), 0.5, material_esfera_1));

    scene.addObject(
        std::make_unique<Prism::Sphere>(Prism::Point3(-0.35, -0.2, -0.6), 0.3, material_esfera_2));
